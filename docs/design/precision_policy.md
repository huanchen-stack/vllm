...
| validation states | `dynamic_switch_rollout_20260822/policies/runtime_full_cost_lookup_committed_250_validation.json` (87 rows) with `runtime_full_cost_receding_1k.json` at scan 250 and `runtime_full_cost_lookup_committed_250.json` | 87/87 `direct == predict_receding_horizon`, 87/87 `lookup == table` |
